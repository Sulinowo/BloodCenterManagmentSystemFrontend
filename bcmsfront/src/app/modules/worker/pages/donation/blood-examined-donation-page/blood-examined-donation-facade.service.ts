...
export class BloodExaminedDonationFacadeService {
  ...
  public isExaminationNullable$ = this.profileFacade.donationDetails$.pipe(map((x: DonationDetails) =>  {
    return (Object.keys(x).length === 0 || x.resultOfExamination === null || x.resultOfExamination.height === 0); // 0 zamiast null
  }));
  constructor(
